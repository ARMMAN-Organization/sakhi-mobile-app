...
@Composable
internal fun HomeContent(summary: DashboardSummary) {
...
private fun SakhiRow(summary: DashboardSummary) {
  ...
  Column(modifier = Modifier.fillMaxWidth()) {
    // Name is centered against the pill itself; the caption hangs below the row.
    Row(
...
private fun DataUploadPill(pendingCount: Int) {
  Button(onClick = { /* no-op: upload flow not built yet */ }) {
    if (pendingCount > 0) {
      Box(
        contentAlignment = Alignment.Center,
        modifier = Modifier.size(22.dp).background(White, CircleShape),
      ) {
        Text(
...
private fun ActiveBeneficiariesCard(summary: DashboardSummary) {
  ...
  SummaryCard(title = stringResource(R.string.home_active_beneficiaries)) {
    Row(
      ...
      StatTile(
        icon = Icons.Filled.Person,
        value = totalWithRisk(b.mothersTotal, b.mothersHighRisk),
        caption = buildAnnotatedString { append(stringResource(R.string.home_mothers)) },
        modifier = Modifier.weight(1f),
      )
      StatTile(
        icon = Icons.Filled.Face,
        value = totalWithRisk(b.infantsTotal, b.infantsHighRisk),
...
/** Formats "total | highRisk" with the high-risk count in red. */
private fun totalWithRisk(total: Int, highRisk: Int) = buildAnnotatedString {
  append(total.toString())
  withStyle(SpanStyle(color = NeutralG200)) { append(" | ") }
  withStyle(SpanStyle(color = RiskHigh)) { append(highRisk.toString()) }
}

/** Fixed bottom bar: All Beneficiaries + Register New (both no-op for now). */
@Composable
  ...
  Surface(color = White, shadowElevation = 8.dp) {
    Row(
      ...
      SecondaryButton(
        ...
        onClick = { /* no-op: beneficiary list not built yet */ },
        modifier = Modifier.weight(1f),
      )
      SecondaryButton(
